import { BaseChart, echarts, Graphic } from '@fanciers/echarts-react';
...
export const GraphicStrokeAnimation: Story = {
  name: 'Stroke Animation',
  render() {
    return (
      <BaseChart style={{ width: 720, height: 500 }}>
        <Graphic
          graphic={{
            elements: [
              {
                ...
                keyframeAnimation: {
                  ...
                  keyframes: [
                    ...
                    { percent: 0.8, style: { fill: 'transparent' } }, // Stop for a while.
...
export const GraphicLoading: Story = {
  name: 'Customized Loading Animation',
  render() {
    return (
      <BaseChart style={{ width: 720, height: 500 }}>
        <Graphic
...
type GraphicComponentElements = GraphicComponentOption extends { mainType?: string; elements?: infer P } ? P : never;

export const GraphicWaveAnimation: Story = {
  name: 'Wave Animation',
  render() {
    ...
    let [noise] = useState(() => {
      const n = getNoiseHelper();
      n.seed(Math.random());
      return n;
    });

    function createElements() {
      const elements: GraphicComponentElements = [];
      let width = 720;
      let height = 500;
      if (chartRef.current) {
        width = chartRef.current.getWidth();
        height = chartRef.current.getHeight();
      }
    ...
    return (
      <BaseChart style={{ width: 720, height: 500 }} backgroundColor={config.backgroundColor}>
        <Graphic
          graphic={{
