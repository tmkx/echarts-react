...
export const MatrixMiniBarGeo: Story = {
  ...
  render() {
    ...
    const matrix: MatrixComponentOption = {
      x: {
        ...
        data: _colHeaders.map(function (item, colIdx): MatrixDimensionCellLooseOption {
          return {
            value: item,
            size: colIdx === _geoColIdx ? '15%' : colIdx === _regionColIdx ? 120 : 0,
          };
        }),
    ...
    for (var dataColIdx = 0; dataColIdx < _colHeaders.length; ++dataColIdx) {
      ...
      for (var dataRowIdx = 0; dataRowIdx < rowCount; ++dataRowIdx) {
        if (dataColIdx === _regionColIdx) {
          addCellPlainText(_dataSourceList, dataColIdx, dataRowIdx);
        } else if (dataColIdx === _geoColIdx) {
          addCellMiniGeo(_dataSourceList, dataColIdx, dataRowIdx);
        } else {
          addCellMiniBar(_dataSourceList, dataColIdx, dataRowIdx, dataExtentOnCol);
        }
      }
    }
    ...
    function addCellMiniGeo(dataSourceList: { name: string; data: (string | number)[][] }[], dataColIdx, dataRowIdx) {
      ...
      geo.push({
        ...
        selectedMode: false,
        tooltip: { show: false },
        regions: [{ name: regionName, selected: true, select: { itemStyle: { color: '#0a41e6' } } }],
        select: { label: { show: false } },
      });
    }
    return (
      <BarChart style={{ width: 480, height: 300 }} grid={grid} xAxis={xAxis} yAxis={yAxis} series={series}>
        <Legend legend={{}} />
        <Tooltip tooltip={{}} />
